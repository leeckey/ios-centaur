package ghostcat.ui
{
    ...
    public class ToolTipSprite extends GBase
    {
        ...
        protected override function init() : void
        {
            ...
            _tickTimer = new Timer(500, int.MAX_VALUE - 1);
            _tickTimer.addEventListener(TimerEvent.TIMER, onTimer);
            _tickTimer.start();
        }
...
//            hide();
        ...
        {
            refreshTooltip(event.target as DisplayObject);
        }
        ...
        private function refreshTooltip(targetObj:DisplayObject):void
        {
            // 如果有对象在拖拽中时，不显示悬浮框
            if (DragManager.hasDragObject())
            {
                hide();
                return;
            }
            
            var newTarget:DisplayObject = findToolTipTarget(targetObj);
            if (newTarget != target)
            {
                target = newTarget;
        ...
        {
            if ((target is IToolTipManagerClient) && (content is IToolTipSkin))
            {
                var client:IToolTipManagerClient = target as IToolTipManagerClient;
                var tooltipData:* = client.toolTip;
                if (tooltipData)
                    (content as IToolTipSkin).data = tooltipData;
                else
                    hide();
            }
        }
        ...
        public override function destory() : void
        {
            ...
            _tickTimer.removeEventListener(TimerEvent.TIMER, onTimer);
            _tickTimer.stop();
            _tickTimer = null;
