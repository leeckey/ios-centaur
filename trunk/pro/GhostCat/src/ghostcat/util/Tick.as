package ghostcat.util
{
    ...
    public class Tick extends Singleton
    {
        ...
        *  Tick回调函数
        */
        protected var _tickCallbacks:Vector.<Function> = new Vector.<Function>();
        
        /**
        * 
        */
        protected var _tickLength:int = 0;
        
        /**
        *  tick事件
        */ 
        private var _tickEvent:TickEvent = new TickEvent(TickEvent.TICK);
        
        /**
        *  tick参数
        */ 
        private var _tickParam:Array = [_tickEvent];
        ...
        *  添加Tick处理函数
        */ 
        public function addTickHandler(handler:Function):void
        {
            if (_tickCallbacks.indexOf(handler) != -1)
                return;
            
            _tickCallbacks[_tickCallbacks.length] = handler;
            _tickLength = _tickCallbacks.length;
        }
        
        /**
         *  移除Tick处理函数
         */
        public function removeTickHandler(handler:Function):void
        {
            var pos:int;
            if ((pos = _tickCallbacks.indexOf(handler)) == -1)
                return;
            
            _tickCallbacks.splice(pos, 1);
            _tickLength = _tickCallbacks.length;
        }
        
        public function hasTickHandler(handler:Function):Boolean
        {
            return (handler != null) && (_tickCallbacks.indexOf(handler) != -1);
        }
        ...
        private function enterFrameHandler(event:Event):void
        {
            ...
            if (!pause)
            {
                ...
                else
                {
                    ...
                    _tickEvent.interval = interval * speed;
                    dispatchEvent(_tickEvent);

                    // 改用回调，提高性能
                    for each (var handler:Function in _tickCallbacks)
                        handler.apply(null, _tickParam);
                }
            }
            prevTime = nextTime;
        }
