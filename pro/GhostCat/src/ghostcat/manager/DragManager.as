package ghostcat.manager
{
    ...
    public class DragManager
    {
        ...
        *  判断现在是否存在物体拖拽中
        */
        public static function hasDragObject():Boolean
        {
            var hasDragObj:Boolean = false;
            for each (var obj:DragManager in list)
            {
                hasDragObj = true;
                break;
            }
        ...
        protected function startDrag():void
        {
            ...
            Tick.instance.addTickHandler(enterFrameHandler);
            ...
            if (type == CLONE || type == ALPHA_CLONE)
            {
                ...
                // 起始拖住点设置成相对于stage的位置
                dragPos = obj.localToGlobal(new Point(-image.x, -image.y));
        ...
        protected function stopDrag():void
        {
            ...
            Tick.instance.removeTickHandler(enterFrameHandler);
            if (obj.stage)
            {
