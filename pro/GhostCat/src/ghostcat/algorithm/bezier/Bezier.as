...
package ghostcat.algorithm.bezier
{
    ...
    public class Bezier implements IParametric
    {
        ...
        *  获取Bezier曲线点段
        */ 
        public function getPointArray(count:uint):Vector.<Point>
        {
            var pointVec:Vector.<Point> = new Vector.<Point>();
            pointVec.push(_start);
            
            for (var i:int = 0; i < count; ++i)
            {
                var percent:Number = (i + 1) / (count + 1);
                pointVec.push(getPoint(percent));
            }
