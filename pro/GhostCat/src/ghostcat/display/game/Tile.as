package ghostcat.display.game
{
    ...
    public class Tile extends GBase
    {
        ...
        *  每个格子中的排版居中模式
        */
        public var centerMode:String = CenterMode.RECT;
        ...
        public function render():void
        {
            ...
            //过滤显示范围    
            var cRect:Rectangle = new Rectangle(0,0,Geom.ceil(width/contentRect.width),Geom.ceil(height/contentRect.height));
        ...
        {
            var startX:int = i * contentRect.width;
            var startY:int = j * contentRect.height;
            if (centerMode == CenterMode.POINT)
            {
                s.x = startX + s.width * 0.5;
                s.y = startY + s.height * 0.5;
            }
            else if (centerMode == CenterMode.RECT)
            {
                s.x = startX + (contentRect.width - s.width) * 0.5;
                s.y = startY + (contentRect.height - s.height) * 0.5;
            }
            else
            {
                s.x = startX;
